...
Pour offrir une expérience visuelle moderne, fluide et haut de gamme, l'interface utilisateur de Sentinel repose sur l'écosystème **Textualize** :
...
- **`Rich` (Engine de Rendu) :**
  - Rendu TrueColor (16,7M de couleurs), bordures arrondies (`round`), dégradés HSL.
  - Coloration syntaxique du code source et rendu Markdown en temps réel.
- **`Questionary` / `InquirerPy` :**
  - Prompting interactif pour l'onboarding (`sentinel init`) avec navigation au clavier et autocomplétion.

### 🎨 Charte Graphique & Palette "Dark Cyberpunk / Sleek Charcoal"
- **Fond Principal (`$surface`) :** Ardoise Sombre (`#0F172A` / `#1E293B`)
- **Accent Primordial (`$primary`) :** Cyan Électrique (`#00F0FF`)
- **Accent Secondaire (`$accent`) :** Violet Néon (`#BD00FF`)
- **Succès / Sécurité (`$success`) :** Vert Émeraude (`#00FF66`)
- **Alerte Contexte (`$warning`) :** Amber Lumineux (`#FFB000`)
- **Erreurs / Interceptions (`$error`) :** Rouge Coral (`#FF453A`)
